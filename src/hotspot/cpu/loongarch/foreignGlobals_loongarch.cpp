...
void RegSpiller::pd_store_reg(MacroAssembler* masm, int offset, VMReg reg) {
  if (reg->is_Register()) {
    masm->st_d(reg->as_Register(), SP, offset);
  } else if (reg->is_FloatRegister()) {
    masm->fst_d(reg->as_FloatRegister(), SP, offset);
  } else {
    // stack and BAD
  }
}
...
void RegSpiller::pd_load_reg(MacroAssembler* masm, int offset, VMReg reg) {
  if (reg->is_Register()) {
    masm->ld_d(reg->as_Register(), SP, offset);
  } else if (reg->is_FloatRegister()) {
    masm->fld_d(reg->as_FloatRegister(), SP, offset);
  } else {
    // stack and BAD
  }
}
...
void ArgumentShuffle::pd_generate(MacroAssembler* masm, VMReg tmp, int in_stk_bias, int out_stk_bias) const {
  ...
  for (int i = 0; i < _moves.length(); i++) {
    ...
    switch (arg_bt) {
      ...
      case T_CHAR:
      case T_INT:
        masm->simple_move32(from_vmreg, to_vmreg, tmp_reg);
        break;
